...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        // Only seed data if database is completely empty (first deployment only)
        if (ownerRepository.count() == 0) {
            System.out.println("🌱 First deployment detected - initializing database with seed data...");
            initializeData();
    ...
    private void initializeData() {
        try {
            ...
            System.out.println("  ✓ Created 5 pet types");
            ...
            System.err.println("⚠️ Error during data initialization: " + e.getMessage());
            e.printStackTrace();
        }
    }
}